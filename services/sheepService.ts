...
const mapToDb = (sheep: Partial<Sheep>) => {
  ...
  if (sheep.brinco !== undefined) payload.brinco = sheep.brinco?.trim();
  if (sheep.nome !== undefined) payload.nome = sheep.nome?.trim();
  ...
  if (sheep.obs !== undefined) payload.obs = sheep.obs?.trim() || null;
