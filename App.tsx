...
const App: React.FC = () => {
  ...
  const HeaderActions = (
    <div className="flex items-center gap-2">
      <div className={`flex items-center gap-2 px-3 py-1.5 border rounded-full transition-all ${aiStatus === 'online' ? 'bg-indigo-50 border-indigo-100' : 'bg-slate-50'}`}>
        <div className={`w-1.5 h-1.5 rounded-full ${aiStatus === 'online' ? 'bg-indigo-500 animate-pulse' : 'bg-slate-300'}`}></div>
        <span className={`text-[9px] font-black uppercase tracking-widest ${aiStatus === 'online' ? 'text-indigo-600' : 'text-slate-400'}`}>Gemini 3.0</span>
      </div>
